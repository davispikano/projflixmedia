...
async function api(req, res, u) {
  ...
  if (u.pathname === '/api/config/toggle') { const b = await parseBody(req); const allowedKeys = ['embeddedPlayer', 'autoNext', 'autoRescan', 'autoDeleteWatched', 'skipIntro', 'autoSkipIntro']; if (!allowedKeys.includes(b.key)) return json(res, 400, { ok: false, error: 'Chave não permitida' }); config[b.key] = !!b.value; saveConfig(); return json(res, 200, { ok: true }); }
  if (u.pathname === '/api/config/number') { const b = await parseBody(req); const allowedKeys = ['autoNextSeconds', 'doubleTapSeconds', 'autoSkipIntroSeconds']; if (!allowedKeys.includes(b.key)) return json(res, 400, { ok: false, error: 'Chave não permitida' }); const val = Number(b.value); if (!Number.isFinite(val) || val < 1 || val > 300) return json(res, 400, { ok: false, error: 'Valor inválido' }); if (b.key === 'autoNextSeconds') config.autoNextSeconds = Math.max(3, Math.min(30, val)); if (b.key === 'doubleTapSeconds') config.doubleTapSeconds = Math.max(3, Math.min(30, val)); if (b.key === 'autoSkipIntroSeconds') config.autoSkipIntroSeconds = Math.max(2, Math.min(20, val)); saveConfig(); return json(res, 200, { ok: true }); }
